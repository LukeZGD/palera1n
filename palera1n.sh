...
if [ ! -f blobs/"$deviceid"-"$version".der ]; then
    ...
    if [ "$tweaks" = "1" ]; then
        if [ -z "$semi_tethered" ]; then
            ...
            venv/bin/python3 -m pyimg4 img4 extract -i work/kernelcache -p work/kcache.im4p
            if [[ "$deviceid" == "iPhone8"* ]] || [[ "$deviceid" == "iPad6"* ]] || [[ "$deviceid" == *'iPad5'* ]]; then
                venv/bin/python3 -m pyimg4 im4p extract -i work/kcache.im4p -o work/kcache.raw --extra work/kpp.bin
            else
                venv/bin/python3 -m pyimg4 im4p extract -i work/kcache.im4p -o work/kcache.raw
            fi
            ...
            if [[ "$deviceid" == *'iPhone8'* ]] || [[ "$deviceid" == *'iPad6'* ]] || [[ "$deviceid" == *'iPad5'* ]]; then
                venv/bin/python3 -m pyimg4 im4p create -i work/kcache.patched -o work/kcache.im4p -f krnl --extra work/kpp.bin --lzss
            else
                venv/bin/python3 -m pyimg4 im4p create -i work/kcache.patched -o work/kcache.im4p -f krnl --lzss
            fi
